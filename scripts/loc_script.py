...
def fetch_urls(url: str, params: dict = None, root: str = None) -> req.Response:
    """
    Fetches and prints the title of a web page from a URL.
    ...
    try:
        ...
        print(f'Below is the response text for {url}')
        print(f'{resp.text}\n')
        print(f'Above is response text for {url}')
    ...
    try:
        if 'json' in res.headers.get('content-type').lower():
            ...
            with open(f'{filename}.txt', 'w', encoding='utf-8') as f:
                f.write(res.text)
            print(f"File has been written to {Path.cwd()} as {filename}.txt")
    except Exception as e:
